...
CREATE OR REPLACE FUNCTION public.get_all_users_auth()
 RETURNS TABLE(id uuid, email text, created_at timestamp with time zone, last_sign_in_at timestamp with time zone, is_admin boolean, is_blocked boolean, full_name text)
 LANGUAGE plpgsql
 SECURITY DEFINER
