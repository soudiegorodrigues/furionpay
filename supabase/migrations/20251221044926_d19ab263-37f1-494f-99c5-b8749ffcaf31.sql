-- Drop both existing versions of the function to resolve PGRST203 duplicate function error
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'conversion_rate', CASE 
      WHEN (SELECT COUNT(*) FROM pix_transactions WHERE user_id = v_effective_owner_id AND created_at >= v_start_date AND created_at <= v_end_date) > 0 
      THEN ROUND(
        (SELECT COUNT(*) FROM pix_transactions WHERE user_id = v_effective_owner_id AND status = 'paid' AND paid_at >= v_start_date AND paid_at <= v_end_date)::NUMERIC / 
        (SELECT COUNT(*) FROM pix_transactions WHERE user_id = v_effective_owner_id AND created_at >= v_start_date AND created_at <= v_end_date)::NUMERIC * 100, 2
      )
      ELSE 0
    END
  ) INTO v_result;
