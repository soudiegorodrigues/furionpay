-- Dropar funções com assinatura completa
...
-- Criar função user transactions com busca por nome do produto
  ...
  SELECT
    ...
    COALESCE(
      ...
      (SELECT po2.offer_code FROM product_offers po2
       ...
       LIMIT 1),
      (SELECT po3.offer_code FROM product_offers po3
       JOIN products p ON p.id = po3.product_id
       WHERE p.name = pt.product_name AND po3.is_active = true
       LIMIT 1)
    ) AS offer_code,
    COALESCE(
      po_direct.domain,
      (SELECT po2.domain FROM product_offers po2
       ...
       WHERE p.product_code = pt.product_code AND po2.is_active = true
       LIMIT 1),
      (SELECT po3.domain FROM product_offers po3
       JOIN products p ON p.id = po3.product_id
       WHERE p.name = pt.product_name AND po3.is_active = true
       LIMIT 1)
    ) AS offer_domain
...
-- Criar função global transactions com busca por nome do produto
