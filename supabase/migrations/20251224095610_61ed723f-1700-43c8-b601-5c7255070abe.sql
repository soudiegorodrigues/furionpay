...
DROP FUNCTION IF EXISTS public.log_pix_generated_user(uuid, text, numeric, text, text, text, text, text, text, text, text, text, text, text, text, text, text, numeric, numeric, text, text, text, jsonb, text, jsonb);

-- Recreate function with proper date casting
CREATE OR REPLACE FUNCTION public.log_pix_generated_user(
  ...
  p_donor_birthdate text DEFAULT NULL,  -- Accept as TEXT
...
DECLARE
  ...
  v_birthdate date;
BEGIN
  -- Cast birthdate text to date, handling NULL and empty strings
  ...
  INSERT INTO pix_transactions (
    ...
    p_donor_city,
    p_donor_state,
    v_birthdate,  -- Use the properly cast date
    p_fee_percentage,
    p_fee_fixed,
    p_fingerprint_hash,
    p_client_ip,
