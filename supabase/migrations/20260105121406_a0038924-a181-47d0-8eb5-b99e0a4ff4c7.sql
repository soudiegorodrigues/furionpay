-- Drop both versions to start fresh
DROP FUNCTION IF EXISTS public.get_user_transactions_paginated(integer, integer, text, date, date, text, text, text);
...
-- Recreate with correct parameter names and product_code search
CREATE FUNCTION public.get_user_transactions_paginated(
...
BEGIN
  ...
  CASE p_date_filter
    ...
    WHEN 'last30days' THEN
      v_start_date := ((now() AT TIME ZONE 'America/Sao_Paulo')::date - interval '30 days')::timestamptz;
      v_end_date := now();
    WHEN 'thisMonth' THEN
      v_start_date := date_trunc('month', now() AT TIME ZONE 'America/Sao_Paulo')::timestamptz;
      v_end_date := now();
    WHEN 'lastMonth' THEN
      v_start_date := (date_trunc('month', now() AT TIME ZONE 'America/Sao_Paulo') - interval '1 month')::timestamptz;
      v_end_date := date_trunc('month', now() AT TIME ZONE 'America/Sao_Paulo')::timestamptz;
    WHEN 'custom' THEN
