-- Increase statement timeout inside backup functions to avoid 2min DB timeout during large JSON aggregation
...
DECLARE
  ...
  v_pix_transactions jsonb;
  v_withdrawal_requests jsonb;
  v_fee_configs jsonb;
  v_available_domains jsonb;
  v_total_records integer := 0;
BEGIN
  -- Default DB statement_timeout is 2 minutes; full backup can exceed that with ~30k PIX rows.
  -- Extend just for this transaction.
  PERFORM set_config('statement_timeout', '15min', true);

  -- Collect admin_settings
  ...
  -- Collect product_checkout_configs
  ...
  -- Collect pix_transactions (last 30 days only for full backup)
  SELECT COALESCE(jsonb_agg(to_jsonb(t)), '[]'::jsonb) INTO v_pix_transactions
  FROM pix_transactions t
  WHERE created_at >= NOW() - INTERVAL '30 days';
  v_total_records := v_total_records + COALESCE(jsonb_array_length(v_pix_transactions), 0);

  -- Collect withdrawal_requests (last 30 days)
  SELECT COALESCE(jsonb_agg(to_jsonb(t)), '[]'::jsonb) INTO v_withdrawal_requests
  FROM withdrawal_requests t
  WHERE created_at >= NOW() - INTERVAL '30 days';
  v_total_records := v_total_records + COALESCE(jsonb_array_length(v_withdrawal_requests), 0);
  ...
  -- Insert backup with correct column names
  INSERT INTO system_backups (
    ...
    'Backup Completo - ' || to_char(NOW(), 'DD/MM/YYYY HH24:MI'),
    'full',
    NOW(),
    v_admin_settings,
    v_products,
...
DECLARE
  ...
  v_total_records integer := 0;
BEGIN
  -- Also extend timeout defensively (import/export can be heavy depending on size)
  PERFORM set_config('statement_timeout', '15min', true);
  ...
  -- Insert backup with correct column names (no transaction data)
  INSERT INTO system_backups (
    backup_name,
    backup_type,
    backed_up_at,
