-- CORREÇÃO URGENTE: Dropar TODAS as versões existentes da função log_pix_generated_user
-- para resolver o erro PGRST203 de ambiguidade entre funções

-- Dropar versão com p_donor_birthdate text (antiga)
DROP FUNCTION IF EXISTS public.log_pix_generated_user(
  numeric, text, text, text, jsonb, text, uuid, text, numeric, numeric, 
  text, text, text, text, text, text, text, text, text, text, text, text, text, text, jsonb
);

-- Dropar versão com p_donor_birthdate date sem offer_id
DROP FUNCTION IF EXISTS public.log_pix_generated_user(
  numeric, text, text, text, jsonb, text, uuid, text, numeric, numeric, 
  text, text, text, text, text, text, date, text, text, text, text, text, text, text, jsonb
);

-- Dropar versão com p_donor_birthdate date COM offer_id
DROP FUNCTION IF EXISTS public.log_pix_generated_user(
  numeric, text, text, text, jsonb, text, uuid, text, numeric, numeric, 
  text, text, text, text, text, text, date, text, text, text, text, text, text, text, jsonb, uuid
);

-- Recriar a função ÚNICA com offer_id opcional
CREATE OR REPLACE FUNCTION public.log_pix_generated_user(
  p_amount NUMERIC,
  p_pix_code TEXT,
  p_txid TEXT,
  p_status TEXT,
  p_utm_data JSONB DEFAULT NULL,
  p_popup_model TEXT DEFAULT NULL,
  p_user_id UUID DEFAULT NULL,
  p_acquirer TEXT DEFAULT 'efi',
  p_fee_percentage NUMERIC DEFAULT 0,
  p_fee_fixed NUMERIC DEFAULT 0,
  p_donor_name TEXT DEFAULT NULL,
  p_donor_cpf TEXT DEFAULT NULL,
  p_donor_email TEXT DEFAULT NULL,
  p_donor_phone TEXT DEFAULT NULL,
  p_product_name TEXT DEFAULT NULL,
  p_product_code TEXT DEFAULT NULL,
  p_donor_birthdate DATE DEFAULT NULL,
  p_donor_cep TEXT DEFAULT NULL,
  p_donor_street TEXT DEFAULT NULL,
  p_donor_number TEXT DEFAULT NULL,
  p_donor_complement TEXT DEFAULT NULL,
  p_donor_neighborhood TEXT DEFAULT NULL,
  p_donor_city TEXT DEFAULT NULL,
  p_donor_state TEXT DEFAULT NULL,
  p_order_bumps JSONB DEFAULT NULL,
  p_offer_id UUID DEFAULT NULL
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_transaction_id UUID;
  v_brazil_now TIMESTAMPTZ;
  v_brazil_date DATE;
BEGIN
  -- Obter horário de Brasília
  v_brazil_now := NOW() AT TIME ZONE 'America/Sao_Paulo';
  v_brazil_date := v_brazil_now::DATE;
  
  INSERT INTO pix_transactions (
    amount,
    pix_code,
    txid,
    status,
    utm_data,
    popup_model,
    user_id,
    acquirer,
    fee_percentage,
    fee_fixed,
    donor_name,
    donor_cpf,
    donor_email,
    donor_phone,
    product_name,
    product_code,
    donor_birthdate,
    donor_cep,
    donor_street,
    donor_number,
    donor_complement,
    donor_neighborhood,
    donor_city,
    donor_state,
    order_bumps,
    offer_id,
    created_date_brazil
  )
  VALUES (
    p_amount,
    p_pix_code,
    p_txid,
    p_status,
    p_utm_data,
    p_popup_model,
    p_user_id,
    p_acquirer,
    p_fee_percentage,
    p_fee_fixed,
    p_donor_name,
    p_donor_cpf,
    p_donor_email,
    p_donor_phone,
    p_product_name,
    p_product_code,
    p_donor_birthdate,
    p_donor_cep,
    p_donor_street,
    p_donor_number,
    p_donor_complement,
    p_donor_neighborhood,
    p_donor_city,
    p_donor_state,
    p_order_bumps,
    p_offer_id,
    v_brazil_date
  )
  RETURNING id INTO v_transaction_id;
  
  RETURN v_transaction_id;
END;
$$;