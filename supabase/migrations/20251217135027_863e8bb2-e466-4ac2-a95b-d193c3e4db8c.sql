...
-- Recreate with corrected filter logic
...
BEGIN
  ...
  RETURN QUERY
  SELECT 
    TO_CHAR(pt.paid_date_brazil, 'DD/MM') as period_key,
  ...
  WHERE pt.status = 'paid'
    AND pt.paid_date_brazil >= start_date
    AND pt.paid_date_brazil <= brazil_today
    AND (p_user_email IS NULL OR u.email = p_user_email)
  GROUP BY pt.paid_date_brazil
  ORDER BY pt.paid_date_brazil ASC;
END;
$$;
