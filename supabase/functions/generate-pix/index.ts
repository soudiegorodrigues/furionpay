...
// Fetch transaction details to get PIX code
async function fetchTransactionDetails(transactionId: string, apiKey: string, maxAttempts: number = 5): Promise<any> {
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    console.log(`Fetching transaction details, attempt ${attempt}/${maxAttempts}`);
    
    const response = await fetch(`${SPEDPAY_API_URL}/v1/transactions/${transactionId}`, {
      method: 'GET',
      headers: {
    ...
    if (response.ok) {
      ...
      console.log(`Transaction details (attempt ${attempt}):`, JSON.stringify(data, null, 2));
      
      // Check if PIX code is now available
      const pixCode = data.pix?.payload || data.pix?.qr_code || data.pixCode;
      if (pixCode) {
        return data;
      }
    } else {
      console.log(`Failed to fetch transaction details (attempt ${attempt}):`, response.status);
    }
...
serve(async (req) => {
  ...
  try {
    ...
    // Check if PIX code is available in the initial response
    let pixCode = data.pix?.payload || data.pix?.qr_code || data.pixCode || data.qr_code;
    let qrCodeUrl = data.pix?.qr_code_url || data.qrCodeUrl;

    // If PIX code not in initial response, try fetching transaction details
    if (!pixCode) {
      console.log('PIX code not in initial response, polling for transaction details...');
      
      // Wait a bit for SpedPay to process
      await delay(2000);
      
      const transactionDetails = await fetchTransactionDetails(transactionId, apiKey, 5);
      
      if (transactionDetails) {
        pixCode = transactionDetails.pix?.payload || transactionDetails.pix?.qr_code || transactionDetails.pixCode;
        qrCodeUrl = transactionDetails.pix?.qr_code_url || transactionDetails.qrCodeUrl;
      }
    }
    ...
    if (!pixCode) {
      ...
      return new Response(
        JSON.stringify({ 
          error: 'PIX code not generated by payment provider. Please check your SpedPay account configuration.',
          details: 'The transaction was created but no PIX code was returned. This may be a configuration issue with your SpedPay account.',
          rawResponse: data 
        }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
