...
const AdminAuth = () => {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center bg-black relative overflow-hidden">
  ...
  return <>
    ...
    <div className="min-h-screen flex items-center justify-center bg-black p-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -top-32 -right-32 w-[700px] h-[700px] rounded-full animate-pulse" style={{
        background: 'radial-gradient(circle, hsl(var(--primary) / 0.4) 0%, transparent 70%)',
        filter: 'blur(80px)'
      }} />
        
        {/* Secondary glow - bottom left */}
        <div className="absolute -bottom-48 -left-48 w-[600px] h-[600px] rounded-full animate-pulse" style={{
        ...
        {/* Center accent glow */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] rounded-full opacity-30" style={{
        ...
        <div className="absolute top-[30%] left-[10%] w-1 h-1 bg-primary/40 rounded-full animate-float-slow" style={{
        animationDelay: '0.5s'
      }} />
        <div className="absolute bottom-[25%] right-[20%] w-2 h-2 bg-primary/50 rounded-full animate-drift" style={{
        animationDelay: '1s'
      }} />
        <div className="absolute top-[60%] left-[15%] w-1.5 h-1.5 bg-primary/30 rounded-full animate-float" style={{
        animationDelay: '2s'
      }} />
        <div className="absolute bottom-20 left-[30%] w-1 h-1 bg-primary/50 rounded-full animate-float-slow" style={{
        animationDelay: '1.5s'
      }} />
        <div className="absolute top-[15%] right-[30%] w-2 h-2 bg-primary/40 rounded-full animate-drift" style={{
        animationDelay: '0.8s'
      }} />
        
        {/* Geometric accents */}
        <div className="absolute top-[20%] left-[8%] w-20 h-20 border border-primary/10 rounded-full animate-spin-slow" />
        <div className="absolute bottom-[15%] right-[8%] w-32 h-32 border border-primary/5 rounded-full animate-spin-slow" style={{
          ...
          <div className="relative bg-black/60 backdrop-blur-2xl rounded-2xl border border-white/10 p-6 md:p-10 shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
            ...
            {mode === 'signup' && <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="name" type="text" placeholder="Seu nome" value={name} onChange={e => setName(e.target.value)} className="h-12 pl-11 pr-4 text-[15px] bg-white/5 border-white/10 text-white rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white/10 transition-all placeholder:text-white/30" required />
                </div>
              </div>}

            {(mode === 'login' || mode === 'signup' || mode === 'reset-email') && <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="email" type="email" placeholder="seu@email.com" value={email} onChange={e => setEmail(e.target.value)} className="h-12 pl-11 pr-4 text-[15px] bg-white/5 border-white/10 text-white rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white/10 transition-all placeholder:text-white/30" required />
                </div>
              </div>}

            {(mode === 'reset-code' || mode === 'unlock-code') && <div className="space-y-3">
                ...
                {mode === 'reset-code' && <p className="text-xs text-center text-white/40 mt-3">
                    Não recebeu?{' '}
                ...
                }} className="text-primary hover:text-primary/80 font-medium transition-colors" disabled={isSubmitting}>
                      Reenviar código
                    </button>
                  </p>}
                {mode === 'unlock-code' && <p className="text-xs text-center text-white/40 mt-3">
                    Um código de desbloqueio foi enviado para seu email.
                  </p>}
              </div>}

            {(mode === 'login' || mode === 'signup') && <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="h-12 pl-11 pr-12 text-[15px] bg-white/5 border-white/10 text-white rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white/10 transition-all placeholder:text-white/30" required minLength={6} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/60 transition-colors">
            ...
            {mode === 'reset-password' && <>
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-[18px] w-[18px] text-white/40 group-focus-within:text-primary transition-colors" />
                    <Input id="newPassword" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="h-12 pl-11 pr-12 text-[15px] bg-white/5 border-white/10 text-white rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white/10 transition-all placeholder:text-white/30" required minLength={6} />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/60 transition-colors">
                      {showPassword ? <EyeOff className="h-[18px] w-[18px]" /> : <Eye className="h-[18px] w-[18px]" />}
                    </button>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-[18px] w-[18px] text-white/40 group-focus-within:text-primary transition-colors" />
                    <Input id="confirmPassword" type={showConfirmPassword ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="h-12 pl-11 pr-12 text-[15px] bg-white/5 border-white/10 text-white rounded-xl focus:border-primary focus:ring-2 focus:ring-primary/20 focus:bg-white/10 transition-all placeholder:text-white/30" required minLength={6} />
                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/60 transition-colors">
            ...
            {mode === 'login' && <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <Checkbox id="remember" checked={rememberMe} onCheckedChange={checked => setRememberMe(checked as boolean)} className="h-4 w-4 rounded border-white/20 data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
                  <Label htmlFor="remember" className="text-sm text-white/50 cursor-pointer hover:text-white/70 transition-colors">
                    Lembrar-me
                  </Label>
                </div>
                <button type="button" onClick={() => switchMode('reset-email')} className="text-sm text-primary hover:text-primary/80 font-medium transition-colors">
                  Esqueceu a senha?
                </button>
              </div>}

            <Button type="submit" className="w-full h-12 text-[15px] font-semibold rounded-xl bg-gradient-to-r from-primary to-red-500 hover:from-primary/90 hover:to-red-500/90 text-white shadow-lg shadow-primary/30 hover:shadow-xl hover:shadow-primary/40 transition-all duration-300 active:scale-[0.98] border-0" disabled={isSubmitting}>
