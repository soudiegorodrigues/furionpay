...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  if (collapsible === "none") {
    return (
      <div
        className={cn("flex h-full flex-col bg-sidebar text-sidebar-foreground flex-shrink-0", className)}
  ...
  ({ className, onClick, ...props }, ref) => {
    ...
    return (
      <Button
        ...
        variant="ghost"
        size="icon"
        className={cn("h-16 w-16 sm:h-7 sm:w-7", className)}
        onClick={(event) => {
          onClick?.(event);
          toggleSidebar();
        }}
        {...props}
      >
        <PanelLeft className="!h-[18px] !w-[18px] sm:!h-4 sm:!w-4" />
