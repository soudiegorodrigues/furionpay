...
export function PremiacoesSection() {
  ...
  const loadPendingRequests = async () => {
    const { data, error } = await supabase.rpc("get_pending_reward_requests");
    if (!error && data) {
  ...
  const loadSentRequests = async () => {
    const { data, error } = await supabase.rpc("get_sent_reward_requests");
    if (!error && data) {
  ...
  const handleMarkAsSent = async () => {
    ...
    const { error } = await supabase.rpc("mark_reward_sent", {
            ...
            <div className="space-y-4">
              <div className="p-3 bg-muted rounded-lg space-y-1">
                ...
                {selectedRequest.delivery_address && (
                  <p className="text-sm"><strong>Endereço:</strong> {selectedRequest.delivery_address}</p>
                )}
              </div>
