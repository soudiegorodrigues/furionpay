...
export const RankingSection = () => {
  ...
  const loadRanking = async () => {
    ...
    try {
      ...
      const {
        data: rankingData
      } = await supabase.rpc('get_users_revenue_ranking_v2', {
    ...
    <Card>
      ...
      <CardContent>
        {isLoading ? <div className="flex items-center justify-center py-8">
          ...
          </div> : rankingUsers.length === 0 ? <p className="text-center text-muted-foreground py-8">
            ...
            <div className="rounded-md border overflow-x-auto -mx-4 sm:mx-0">
              <Table>
                ...
                <TableBody>
                  {rankingUsers.map((rankUser, index) => <TableRow key={rankUser.user_id || index}>
            ...
            {totalPages > 1 && <div className="flex items-center justify-between mt-4 pt-4 border-t">
                ...
                <div className="flex gap-1 sm:gap-2">
                  <Button variant="outline" size="sm" className="h-8 px-2" onClick={() => handlePageChange(rankingPage - 1)} disabled={rankingPage === 1 || isLoading}>
                    <ChevronLeft className="h-4 w-4" />
                  </Button>
                  <Button variant="outline" size="sm" className="h-8 px-2" onClick={() => handlePageChange(rankingPage + 1)} disabled={rankingPage >= totalPages || isLoading}>
