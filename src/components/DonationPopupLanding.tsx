      ...
      <div className="w-full max-w-lg mx-auto px-3 sm:px-4 py-4 sm:py-10">
        {step === "select" && <div className="space-y-5 sm:space-y-6">
            ...
            <div>
              ...
              <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden mb-3 sm:mb-4">
                ...
                <Input type="text" value={customAmount} onChange={handleCustomAmountChange} className="border-0 text-base sm:text-lg font-medium h-11 sm:h-12 focus-visible:ring-0 bg-white text-gray-900" placeholder="0,00" />
              </div>

              {/* Amount Grid */}
              <div className="grid grid-cols-2 gap-2 sm:gap-3 items-end">
                {DONATION_AMOUNTS.map(item => <div key={item.amount} className="flex flex-col">
                    {item.badge}
                    <button onClick={() => handleSelectAmount(item.amount)} className={cn("py-3 px-3 sm:py-3.5 sm:px-4 rounded-lg border transition-all font-medium text-sm sm:text-base", selectedAmount === item.amount ? "border-[#00A651] bg-[#00A651]/5 text-[#00A651]" : "border-gray-300 bg-white text-gray-700 hover:border-[#00A651]/50")}>
            ...
            <div>
              ...
              return <button key={boost.id} onClick={() => toggleBoost(boost.id)} className={cn("flex flex-col items-center p-2 sm:p-4 rounded-lg transition-all border border-dashed", isSelected ? "border-[#00A651] bg-[#00A651]/5" : "border-gray-300 hover:border-gray-400")}>
                      <div className={cn("w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center mb-1.5 sm:mb-2", boost.bgColor)}>
        ...
        {step === "upsell" && <div className="space-y-5 sm:space-y-6">
            ...
            <div className="bg-[#E8F5E9] rounded-2xl p-4 sm:p-6 text-center">
              ...
              <button onClick={handleUpsellDecline} className="w-full py-3 sm:py-4 px-4 rounded-lg border-2 border-gray-300 bg-white text-gray-700 font-semibold text-sm sm:text-base hover:bg-gray-50 hover:border-gray-400 transition-all">
                Não, obrigado. Continuar com {formatCurrency(getContributionAmount())}
              </button>
            </div>
          </div>}

        {step === "loading" && <div className="bg-white rounded-xl p-4 sm:p-8">
            <PixLoadingSkeleton />
          </div>}

        {step === "pix" && pixData && <div className="bg-white rounded-xl p-4 sm:p-8">
            <PixQRCode amount={calculateTotal()} pixCode={pixData.code} qrCodeUrl={pixData.qrCodeUrl} transactionId={pixData.transactionId} onRegenerate={handleBack} />
          </div>}
      </div>
