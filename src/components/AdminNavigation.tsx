...
export function AdminNavigation({
  ...
  return <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-wrap gap-1.5 sm:gap-2">
        {adminSections.map(section => <Button key={section.id} variant={activeSection === section.id ? "default" : "outline"} size="sm" className="flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm px-2 sm:px-3 h-8 sm:h-9" onClick={() => handleSectionClick(section.id)}>
